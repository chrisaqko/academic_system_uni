  ...
  const columns = [
    {
      ...
      render: (row) => (
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 bg-primary-50 rounded-lg flex items-center justify-center text-primary-600 font-bold text-xs shrink-0">
        ...
        return (
          <div className="flex items-center gap-2 w-28">
            <div className="flex-1 h-1.5 bg-slate-100 rounded-full overflow-hidden">
              <div
                className={`h-full rounded-full ${occ > 80 ? "bg-emerald-500" : occ > 60 ? "bg-amber-500" : "bg-red-400"}`}
                style={{ width: `${occ}%` }}
              />
            </div>
        ...
        <ClassroomFormModal
          ...
          onSave={(r) => {
            if (editRoom) {
              setRooms((prev) =>
                prev.map((p) =>
                  p.id_classrom === editRoom.id_classrom ? { ...p, ...r } : p,
                ),
              );
            } else {
