...
import { getDashboardMetrics, getSchedules } from "@/lib/supabase/queries";
  ...
  return (
    <div className="p-6 space-y-6 animate-fade-in">
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-soft p-5 flex items-center gap-4">
        <Avatar name={profile?.name} surname={profile?.surname} size="md" />
        ...
        <div className="hidden sm:flex flex-col items-end gap-1">
          <span className="inline-flex items-center gap-1.5 text-[11px] font-semibold text-primary-700 bg-primary-50 px-2.5 py-1 rounded-full">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          label="Total Courses"
          value={loadingMetrics ? "…" : (metrics?.totalCourses ?? 0).toLocaleString()}
          sub={`Across ${metrics?.departments ?? "—"} departments`}
          trend="+12%"
          trendPositive
          icon={BookOpen}
          accentColor="blue"
        ...
        <StatCard
          ...
          value={loadingMetrics ? "…" : (metrics?.activeClassrooms ?? 0).toLocaleString()}
          sub={metrics?.classroomOccupancy ? `${metrics.classroomOccupancy} occupancy` : "Live data"}
          trend="STABLE"
          icon={Building2}
          accentColor="violet"
        />
        <StatCard
          ...
          value={loadingMetrics ? "…" : (metrics?.totalStudents ?? 0).toLocaleString()}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
              ...
              (metrics.recentActivity).map((item, i) => {
                ...
                return (
                  <div key={i} className="flex items-start gap-3 py-2.5 border-b border-slate-50 last:border-0">
                    <div className={`p-2 rounded-lg shrink-0 ${iconColors[item.icon] || "bg-slate-100 text-slate-500"}`}>
                      <Icon size={14} />
                    </div>
                    <div className="flex-1 min-w-0">
                    ...
                    <span className="text-[11px] text-slate-300 whitespace-nowrap shrink-0">{item.time}</span>
                  </div>
                );
              })
            )}
        ...
        <div className="bg-white border border-slate-200 rounded-xl shadow-soft p-5">
          ...
          <div className="space-y-4">
            {(metrics?.staffingInsights ?? []).length === 0 ? (
              <p className="text-xs text-slate-400 text-center py-6">No staffing data</p>
            ) : (
              (metrics.staffingInsights).map((dept, i) => (
                <div key={i}>
                  ...
                  <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
                    <div
                      className={`h-full rounded-full transition-all duration-700 ${dept.fill >= 80 ? "bg-primary-600" : dept.fill >= 60 ? "bg-amber-500" : "bg-red-500"}`}
          ...
          <div className="mt-5 pt-4 border-t border-slate-100 bg-primary-50 -mx-5 -mb-5 px-5 pb-5 rounded-b-xl">
            ...
            <p className="text-xs text-slate-600 italic leading-relaxed">
              &ldquo;Increase humanities staffing by 12% to meet projected spring enrollment surge.&rdquo;
            </p>
          </div>
        </div>
