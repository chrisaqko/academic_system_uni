...
import { mockCourses, mockTeacherAllocation, mockEnrolledSchedule, mockUsers } from '@/lib/data/mockData';
...
export default function TeacherDashboard() {
  ...
  useEffect(() => {
    ...
    setSession(s);
    if (s) setSE(buildTeacherSchedule(s.id_user));
  }, []);
  ...
  return (
    <div className="p-6 space-y-6 animate-fade-in">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Faculty Overview</h1>
          <p className="text-sm text-slate-400 mt-0.5">Academic Quarter: Fall 2024 • Week 08</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2">
          <ScheduleGrid entries={scheduleEntries} title="Weekly Teaching Schedule" />
        </div>

        {/* Current class panel */}
        <div className="space-y-4">
          <div className="bg-white border border-slate-200 rounded-xl shadow-soft p-5">
            <h3 className="text-sm font-semibold text-slate-900 mb-3">Current Class</h3>
            <p className="text-xs text-primary-600 font-semibold mb-4">ARCH 101 - Lecture Theatre 02</p>

            <div className="bg-slate-50 rounded-lg p-4 mb-4">
              <div className="flex items-center gap-2 mb-1">
                <Users size={16} className="text-slate-400" />
                <span className="text-2xl font-bold text-slate-900">42 / 45</span>
              </div>
              <p className="text-xs text-slate-400">Students Present</p>
            </div>

            <div className="flex justify-between items-center py-2 border-t border-slate-100">
              <span className="text-xs text-slate-500 font-medium">Session Time Remaining</span>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between py-2 px-3 bg-amber-50 border border-amber-100 rounded-lg">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-medium text-amber-700">3 Unmarked Assignments</span>
                  </div>
                ...
                <div className="flex items-center justify-between py-2 px-3 bg-slate-50 border border-slate-100 rounded-lg">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-medium text-slate-700">12 New Messages</span>
                  </div>
                  <span className="text-slate-300 text-xs">›</span>
                </div>
              </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-base font-semibold text-slate-900">Active Courses & Enrollment</h3>
          <button className="text-xs text-primary-600 font-medium hover:underline">View All Programs</button>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {TEACHER_COURSES.map(course => (
            <div key={course.id} className="bg-white border border-slate-200 rounded-xl shadow-soft overflow-hidden hover:shadow-card hover:border-slate-300 transition-all duration-200 cursor-pointer">
              <div className="relative h-36">
                <img src={course.img} alt={course.name} className="w-full h-full object-cover" />
                <div className="absolute inset-0 bg-gradient-to-t from-slate-900/60 to-transparent" />
                <span className={`absolute top-3 left-3 px-2 py-0.5 text-[10px] font-bold text-white rounded uppercase tracking-wider ${levelColors[course.level] || 'bg-slate-700/80'}`}>
                  {course.level}
                </span>
              </div>
              <div className="p-4">
                <h4 className="text-sm font-semibold text-slate-900 mb-3">{course.name}</h4>
                <div className="flex items-center justify-between">
                  <div className="flex -space-x-2">
                    {['#6366f1','#10b981','#f59e0b'].map((c, i) => (
                      <div key={i} className="w-6 h-6 rounded-full border-2 border-white flex items-center justify-center text-[10px] text-white font-bold" style={{ backgroundColor: c }}>
                        {['A','B','C'][i]}
                      </div>
                    ))}
                    {course.students > 3 && (
